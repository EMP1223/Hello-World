amdpCluster = function(apdp_obj, nClusters, plot = TRUE, plot_margin = 0.05, colorvec, plot_pdp=FALSE,
	...
	if(missing(colorvec)){
		colorvec = c("red", "green", "blue", "yellow2", "black", "violetred4", "cyan", "darkgrey", "orange2", "bisque3")
	...
	if(plot){
		...
		#x grid
		grid = apdp_obj$gridpts 
		if(x_quantile){
			ecdf_fcn = ecdf(grid)
			grid = ecdf_fcn(grid)
		}
		plot(grid, cl$centers[1,], type='n', ylim = c(rg_min, rg_max), ylab = paste("cluster",expression(hat(y))))
...
#source("/home/alex/workspace/advanced_marginal_dependence_plots/amdp/R/amdpCluster.R")
